    ...
    public void insertUpdateDeleteStudent(char operation, Integer id, String label, Integer hours){
        ...
        if(operation=='i'){
            try {
                ...
                if(ps.executeUpdate()>0){
                    JOptionPane.showMessageDialog(null, "Khóa học mới đã được thêm");
                }
        ...
        if(operation=='u'){
            try {
                ...
                ps.setInt(3, id);
                if(ps.executeUpdate()>0){
                    JOptionPane.showMessageDialog(null, "Thông tin khóa học đã được cập nhật");
                }
            } catch (SQLException ex) {
                Logger.getLogger(student.class.getName()).log(Level.SEVERE, null, ex);
            }
        ...
        if(operation=='d'){
            ...
            try {
                ...
                ps.setInt(1, id);
                if(ps.executeUpdate()>0){
                    JOptionPane.showMessageDialog(null, "Thông tin khóa học đã được xóa");
                }
            } catch (SQLException ex) {
                Logger.getLogger(student.class.getName()).log(Level.SEVERE, null, ex);
            }
