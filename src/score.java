      ...
      public void insertUpdateDeleteStudent(char operation, Integer Sid, Integer Cid, Double scr, String descp){
        ...
        if(operation=='i'){
            try {
                ...
                if(ps.executeUpdate()>0){
                    JOptionPane.showMessageDialog(null, "Điểm số đã được thêm");
                }
        ...
        if(operation=='u'){
            try {
                ...
                ps.setInt(4, Cid);
                if(ps.executeUpdate()>0){
                    JOptionPane.showMessageDialog(null, "Điểm khóa học đã được cập nhật");
                }
            } catch (SQLException ex) {
                Logger.getLogger(student.class.getName()).log(Level.SEVERE, null, ex);
            }
        ...
        if(operation=='d'){
            try {
                ...
                ps.setInt(2, Cid);
                if(ps.executeUpdate()>0){
                    JOptionPane.showMessageDialog(null, "Điểm khóa học đã được xóa");
                }
            } catch (SQLException ex) {
                Logger.getLogger(student.class.getName()).log(Level.SEVERE, null, ex);
            }
